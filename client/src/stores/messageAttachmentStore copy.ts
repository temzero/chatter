...
>((set, get) => ({
  ...
  fetchMoreAttachments: async (chatId: string, type?: AttachmentType) => {
    console.log("fetchMoreAttachments called", { chatId, type });
    ...
    if (!state.hasMore[chatId]) {
      console.log("Aborting: no more attachments to fetch for this chat");
      return 0;
    }
    ...
    try {
      ...
      const lastAttachment =
        filteredAttachments[filteredAttachments.length - 1];
      const lastId = lastAttachment?.id; // API pagination key
      console.log("Last attachment id:", lastId);
      ...
      if (response.items.length > 0) {
        console.log("Adding attachments to state:", response.items);
        addAttachmentsToState(chatId, response.items);
        ...
        console.log(
          "Updated state with new attachments and hasMore =",
          response.hasMore
        );
      } else {
        set((state) => ({
        ...
        console.log("No new attachments, set hasMore false");
      }
...
export const useHasMoreForType = (chatId: string, type?: AttachmentType) => {
  return useAttachmentStore((state) => {
    ...
    if (typeAttachments.length === 0) {
      return false; // No attachments yet, might have some
    }
