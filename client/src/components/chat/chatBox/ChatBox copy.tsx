...
const ChatBox: React.FC<ChatBoxProps> = ({ chat }) => {
  ...
  const handleInfiniteScroll = useCallback(
    async (e: React.UIEvent<HTMLDivElement>) => {
      ...
      // Only trigger if very close to the top
  ...
  // Auto-fill container when too few messages to scroll
  useEffect(() => {
    const tryFillContainer = async () => {
      ...
      // Keep fetching until we can scroll or no more messages
      while (
        hasMoreMessages &&
    ...
    tryFillContainer();
  }, [chatId, messages.length, hasMoreMessages, fetchMoreMessages]);
  ...
  // Auto scroll if near bottom on new message
  useEffect(() => {
