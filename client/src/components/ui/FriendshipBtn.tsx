...
const FriendshipBtn: React.FC<FriendshipBtnProps> = ({
  ...
  async function handleAcceptRequest() {
    ...
    try {
      ...
      await respondToRequest(receivedRequest.id, FriendshipStatus.ACCEPTED);
  ...
  async function handleDeclineRequest() {
    if (!receivedRequest) return;
    try {
      await respondToRequest(receivedRequest.id, FriendshipStatus.DECLINED);
      onStatusChange?.(null);
    } catch (error) {
  ...
  async function handleCancelRequest() {
    ...
    try {
      await removeRequest(sentRequest.id, userId);
      onStatusChange?.(null);
    } catch (error) {
